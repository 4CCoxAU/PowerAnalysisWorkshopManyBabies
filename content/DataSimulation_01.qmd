...
* the average ADS intercept capturing the mean baseline tendency to attend to ADS, \beta^{ADS}
* the average influence of IDS on this baseline attention, \beta^{IDS}
...
And lastly:
* a trial-level residual error \varepsilon
...
subject_varyingintercept   <- 750 # by-subject random intercept sd
item_varyingintercept <- 500   # by-item random intercept sd
```
...
subject_varyingslope  <-  500 # by-subject random slope sd
