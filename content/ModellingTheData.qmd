...
EffectSizeDataSimulated %>%
  ...
  xlim(c(-5*mean_slope, 5*mean_slope)) +
...
sims <- read_csv("EFsimulations.csv") %>%
...
# read saved simulation data
sims <- read_csv("EFsimulations2Stimuli.csv") %>%
  dplyr::select(term, estimate, std.error, p.value)
...
# read saved simulation data
sims <- read_csv("EFsimulationsAge.csv") %>%
  dplyr::select(term, estimate, std.error, p.value)
